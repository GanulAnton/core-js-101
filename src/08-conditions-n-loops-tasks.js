...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i++) if (str.indexOf(str[i]) === i && str.lastIndexOf(str[i]) === i) return str[i];
  return null;
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m1.length; i++) {
    ...
    for (let j = 0; j < m2[i].length; j++) {
      result[i][j] = 0;
      for (let t = 0; t < m1[i].length; t++) result[i][j] += m1[i][t] * m2[t][j];
    }
  }
  return result;
}
...
function evaluateTicTacToePosition(position) {
  ...
  for (const row of win) {
    let b = true;
    const v = position[row[0][0]][row[0][1]];
    if (v !== undefined) {
      for (let i = 1; i < row.length; i++) b = b && v === position[row[i][0]][row[i][1]];
      if (b) return v;
    }
  }
}
